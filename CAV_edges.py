...
class CAVVisualizer:
    ...
    def create_comparison_plot(self, transfers, strengths, content_img, style_img, 
        ...
        for idx, (transfer, strength) in enumerate(zip(transfers, strengths)):
            ...
            ax.set_title(f'edges Strength: {strength:.1f}')
            ax.axis('off')
        ...
        plt.colorbar(sm, cax=cbar_ax, label='edges Strength')
        
        plot_filename = f'edges_comparison_content{content_name}_style_{style_num:02d}.png'
        plot_path = os.path.join(save_dir, plot_filename)
        plt.savefig(plot_path, bbox_inches='tight', dpi=300)
        plt.close()

class edgesCAVController:
    ...
    def collect_edges_images(self, style_dir):
        ...
        edges_dir = os.path.join('data', 'processed_styles', 'edges_styles')  # Correct path initialization
        
        if not os.path.exists(edges_dir):
            raise ValueError(f"Edges styles directory not found: {edges_dir}")
        
        edges_list = []  # Temporary list to store data
        for item in os.listdir(edges_dir):
            if item.startswith('edges_') and os.path.isdir(os.path.join(edges_dir, item)):
                try:
                    intensity = float(item.split('_')[1])
                    edges_list.append((intensity, item))
                except ValueError:
        ...
        edges_list.sort(key=lambda x: x[0])  # Sort by intensity
        
        image_paths = []  # Collect valid image paths
        for intensity, edges_dir_name in edges_list:
            full_dir = os.path.join(edges_dir, edges_dir_name)
            # Format intensity to match file name (1 decimal place as an example)
            formatted_intensity = f"{intensity:.1f}"
            image_name = f"{style_num}_edges_{formatted_intensity}.png"
    ...
    def learn_edges_cav(self, style_dir):
        try:
            # Get image paths
            original_path, edges_paths = self.collect_edges_images(style_dir)
            ...
            with tqdm(total=3, desc="Learning CAV", leave=False) as pbar:
                ...
                # Get features for edgesed images
                edges_features = []
                edges_paths = [edges_paths[0], edges_paths[-1]]
                for path in edges_paths:
                    features = self.get_style_features(path)
                    edges_features.append(features.view(features.size(0), -1).cpu().numpy())
                pbar.update(1)
                
                # Prepare and train SVM
                edges_flat = np.concatenate(edges_features, axis=0)
                features = np.concatenate([original_flat, edges_flat])
                labels = np.concatenate([
                    np.zeros(len(original_flat)),
                    np.ones(len(edges_flat))
                ])
...
def main():
    ...
    cav_controller = edgesCAVController(vgg, matrix, dec, opt.layer)
    ...
    for content_idx, (content, contentName) in enumerate(tqdm(content_loader, desc="Processing content images")):
        ...
        for style_idx, (style, styleName) in enumerate(tqdm(style_loader, desc=f"Processing styles for content {content_idx+1}", leave=False)):
            ...
            try:
                # Learn CAV
                edges_cav = cav_controller.learn_edges_cav(style_dir)
                
                # Generate variations with progress bar
                transfers = []
                for strength in tqdm(opt.cav_strengths, desc="Generating CAV variations", leave=False):
                    transfer, _ = cav_controller.apply_cav(contentV, styleV, edges_cav, strength)
