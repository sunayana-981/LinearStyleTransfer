...
parser.add_argument("--style_layers", default="r41", help='layers for style')
...
for iteration in range(1, opt.niter + 1):
    ...
    print("VGG Feature dimensions:")
    for k, v in cF.items():
        print(f"Layer {k}: {v.size()}")

    total_loss = 0
    for layer in matrix:
        # Forward through the transformation matrix and decoder
        print(f"\nProcessing layer: {layer}")
        print(f"Content feature shape: {cF[layer].size()}")
        print(f"Style feature shape: {sF[layer].size()}")
        
        feature, transmatrix = matrix[layer](cF[layer], sF[layer])
        print(f"After MulLayer shape: {feature.size()}")
        
        transfer = dec(feature)
        print(f"After decoder shape: {transfer.size()}")
